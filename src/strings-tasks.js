...
function countVowels(str) {
  return str ? (str.match(/[aeiouyAEIOUY]/g) || []).length : 0;
}
...
function extractNameFromTemplate(value) {
  return value.match(/^Hello, ([A-Za-z\s]+)!$/)[1];
}
...
function getCardId(value) {
  ...
  return (
    suits.findIndex((x) => x === value.slice(-1)) * 13 +
    ranks.findIndex((x) => x === value.slice(0, -1))
  );
}
